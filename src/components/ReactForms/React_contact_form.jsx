    ...
    <div className="border-2 border-purple-400 bg-gradient-to-r from-gray-950 via-gray-900 to-gray-950 shadow-md shadow-purple-500/30 p-5 sm:p-10 rounded-2xl">
      ...
      >
        ...
        <div className="flex flex-col gap-y-2">
          <textarea
            {...register("Message", {
              required: "Message Box is Empty",
              validate: (value) =>
                value.trim() !== "" || "Message Box is Empty",
            })}
            cols="40"
            rows="5"
            placeholder="Message"
        ...
        <button
          type="submit"
          value="Send Message"
          className="text-white py-2 rounded-lg cursor-pointer bg-gradient-to-r from-purple-700 to-blue-600 transition-all duration-300 ease-in-out hover:opacity-80 flex justify-center gap-x-3 font-semibold"
        >
          Send Message
        ...
        {status.message && (
          <p
            className={`${status.type === "success" ? "text-white" : "text-red-600"} font-semibold text-sm`}
          >
            {status.message}
          </p>
        )}
